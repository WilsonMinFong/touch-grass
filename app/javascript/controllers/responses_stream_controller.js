    ...
    handleNewResponseData(data) {
        ...
        if (list) {
            ...
            if (existing) {
                ...
                const currentAuthorToken = this.currentAuthorTokenValue
                const isMe = data.author_token === currentAuthorToken

                const chatClass = isMe ? 'chat-end' : 'chat-start'
                const bubbleClass = isMe ? 'chat-bubble-primary' : 'chat-bubble-secondary'

                const html = `
          <div class="chat ${chatClass}" id="response-${data.response_id}">
            <div class="chat-bubble ${bubbleClass} flex flex-col gap-1 min-w-[120px]">
              ...
              <div class="self-end -mb-1 -mr-1">
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 transition-transform duration-200 text-white/70" data-reaction-target="icon" viewBox="0 0 56 56" fill="currentColor">
